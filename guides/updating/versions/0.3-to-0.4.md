...
As noted in the [Update Indy SDK to Askar guide](../update-indy-sdk-to-askar.md), it is very important that the 0.3.x to 0.4.x is started after migrating from the Indy SDK to Aries Askar.
...
It is advised to use the default value as configured by the agent (V1). If you want to keep the old behaviour, you can [configure the `didCommMimeType` property in the agent configuration](/guides/tutorials/agent-config#didcommmimetype).
