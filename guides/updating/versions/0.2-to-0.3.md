...
- `Dispatcher.registerHandler` is now `Dispatcher.registerMessageHandlers` and is marked as deprecated. The recommended way of registering handlers is by using the new `MessageHandlerRegistry` object by calling `MessageHandlerRegistry.registerMessageHandlers`.
